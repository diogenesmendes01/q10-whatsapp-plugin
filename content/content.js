/* ============================================================
   ...
   Injects side panel + detects selected conversation phone.
   v2.0 — Enrollment wizard + student financials
   ============================================================ */

(function () {
  'use strict';
  if (document.getElementById('q10-panel')) return;
  ...
  let wizardState = null; // { step, phone, data: { contacto, estudiante, inscripcion, matricula, orden } }
  ...
  function createPanel() {
    const panel = el('div', 'q10-panel q10-hidden');
    panel.id = 'q10-panel';
    panel.innerHTML = `
      <div class="q10-header">
        <div class="q10-header-left">
          <span class="q10-header-logo">${ICONS.logo}</span>
          <div>
            <div class="q10-header-title">Q10 CRM</div>
            <div class="q10-header-subtitle">Genius Idiomas</div>
          </div>
        </div>
        <button class="q10-btn-close" id="q10-close-btn">${ICONS.close}</button>
      </div>
      <div class="q10-body" id="q10-body"></div>
      <div class="q10-actions" id="q10-actions" style="display:none;"></div>
    `;
    document.body.appendChild(panel);
    document.getElementById('q10-close-btn').addEventListener('click', togglePanel);
    return panel;
  }

  function createToggleButton() {
    const btn = el('button', 'q10-toggle-btn');
    btn.id = 'q10-toggle-btn';
    btn.title = 'Q10 CRM';
    btn.innerHTML = `<span class="q10-toggle-icon">${ICONS.logo}</span>`;
  ...
  function togglePanel() {
    ...
    document.getElementById('q10-panel').classList.toggle('q10-hidden', !panelVisible);
  }
  ...
  // ================================================================
  //  RENDER: ESTUDIANTE (with financials)
  // ================================================================
          ...
          <div class="q10-info-card">
            <div class="q10-info-row">
              ...
              <span class="q10-info-value">
                <span class="q10-badge ${saldo > 0 ? 'q10-badge-red' : 'q10-badge-green'}">
                  ${fmtMoney(saldo)}
                </span>
              </span>
            </div>
            ${ec.Total_pagado !== undefined ? `<div class="q10-info-row">
              <span class="q10-info-label">Total Pagado</span>
              <span class="q10-info-value">${fmtMoney(ec.Total_pagado)}</span>
            </div>` : ''}
            ${ec.Total_cobrado !== undefined ? `<div class="q10-info-row">
              <span class="q10-info-label">Total Cobrado</span>
              <span class="q10-info-value">${fmtMoney(ec.Total_cobrado)}</span>
            </div>` : ''}
          </div>
        </div>`;
    }
        ...
        <div class="q10-section">
          ...
          ${result.pagosPendientes.map(p => `
            <div class="q10-info-card" style="border-left:3px solid #EF4444;">
              <div class="q10-info-row">
                <span class="q10-info-label">Concepto</span>
                <span class="q10-info-value">${p.Concepto || p.Descripcion || p.Nombre || '—'}</span>
              </div>
              <div class="q10-info-row">
                <span class="q10-info-label">Valor</span>
                <span class="q10-info-value"><span class="q10-badge q10-badge-red">${fmtMoney(p.Valor || p.Monto || p.Saldo)}</span></span>
              </div>
              ${p.Fecha_vencimiento ? `<div class="q10-info-row">
                <span class="q10-info-label">Vencimiento</span>
                <span class="q10-info-value">${new Date(p.Fecha_vencimiento).toLocaleDateString('es')}</span>
              </div>` : ''}
            </div>
          `).join('')}
        </div>`;
  ...
  function renderResult(result) {
    ...
    const toggleBtn = document.getElementById('q10-toggle-btn');
    switch (result.type) {
      case 'estudiante':
        renderEstudiante(result);
        toggleBtn.classList.add('q10-has-data');
        break;
      case 'oportunidad':
        ...
        break;
      case 'contacto':
        renderContacto(result.data);
        toggleBtn.classList.add('q10-has-data');
        break;
      default:
        renderUnknown(result.phone);
        toggleBtn.classList.remove('q10-has-data');
    }
  }
  ...
  async function startEnrollmentWizard(phone, existingData) {
    wizardState = {
      ...
      results: {} // stores API responses per step
    };
    ...
    switch (step) {
      case 0: // Contacto
        formHtml = `
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Nombre *</label>
            <input class="q10-form-input" id="wz-fname" value="${pf.Primer_nombre||''}" placeholder="Nombre">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Segundo Nombre</label>
            <input class="q10-form-input" id="wz-fname2" value="${pf.Segundo_nombre||''}" placeholder="Segundo nombre">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Apellido *</label>
            <input class="q10-form-input" id="wz-lname" value="${pf.Primer_apellido||''}" placeholder="Apellido">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Segundo Apellido</label>
            <input class="q10-form-input" id="wz-lname2" value="${pf.Segundo_apellido||''}" placeholder="Segundo apellido">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Número de Identificación</label>
            <input class="q10-form-input" id="wz-docnum" value="${pf.Numero_identificacion||''}" placeholder="CPF / Cédula / Pasaporte">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Email</label>
            <input class="q10-form-input" id="wz-email" type="email" value="${pf.Email||''}" placeholder="email@ejemplo.com">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Celular</label>
            <input class="q10-form-input" id="wz-phone" value="${wizardState.phone||''}" placeholder="+55...">
          </div>
        `;
        break;
      ...
      case 1: // Estudiante
        formHtml = `
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Tipo de Identificación</label>
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Fecha de Nacimiento</label>
            <input class="q10-form-input" id="wz-birthdate" type="date">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Género</label>
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Dirección</label>
            <input class="q10-form-input" id="wz-address" placeholder="Dirección completa">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Observaciones</label>
            <textarea class="q10-form-textarea" id="wz-est-obs" placeholder="Notas adicionales..."></textarea>
          </div>
        `;
        break;
      ...
      case 2: // Inscripción
        {
          const programasOpts = (catalogsCache?.programas || [])
            .map(p => `<option value="${p.Codigo}">${p.Nombre || p.Descripcion || p.Codigo}</option>`)
            .join('');
          ...
          formHtml = `
            ...
            <div class="q10-form-group">
              ...
              <select class="q10-form-select" id="wz-programa">
                <option value="">— Seleccionar programa —</option>
                ${programasOpts || '<option value="" disabled>No hay programas disponibles</option>'}
              </select>
            </div>
            <div class="q10-form-group">
              <label class="q10-form-label">Periodo *</label>
              <select class="q10-form-select" id="wz-periodo">
                <option value="">— Seleccionar periodo —</option>
                ${periodosOpts || '<option value="" disabled>No hay periodos disponibles</option>'}
              </select>
            </div>
            <div class="q10-form-group">
              <label class="q10-form-label">Jornada</label>
              <select class="q10-form-select" id="wz-jornada">
            ...
            <div class="q10-form-group">
              <label class="q10-form-label">Observaciones</label>
              <textarea class="q10-form-textarea" id="wz-insc-obs" placeholder="Notas sobre la inscripción..."></textarea>
            </div>
          `;
        }
        break;
      ...
      case 3: // Matrícula
        {
          ...
          formHtml = `
            ...
            <div class="q10-form-group">
              <label class="q10-form-label">Estado de Matrícula</label>
              <select class="q10-form-select" id="wz-mat-estado">
            ...
            <div class="q10-form-group">
              <label class="q10-form-label">Observaciones</label>
              <textarea class="q10-form-textarea" id="wz-mat-obs" placeholder="Notas sobre la matrícula..."></textarea>
            </div>
          `;
        }
        break;
      ...
      case 4: // Cobro
        formHtml = `
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Concepto *</label>
            <input class="q10-form-input" id="wz-cobro-concepto" value="Matrícula" placeholder="Ej: Matrícula, Mensualidad">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Valor *</label>
            <input class="q10-form-input" id="wz-cobro-valor" type="number" min="0" step="0.01" placeholder="0.00">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Fecha de Vencimiento *</label>
            <input class="q10-form-input" id="wz-cobro-fecha" type="date" value="${new Date(Date.now()+30*86400000).toISOString().split('T')[0]}">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Observaciones</label>
            <textarea class="q10-form-textarea" id="wz-cobro-obs" placeholder="Detalles del cobro..."></textarea>
          </div>
        `;
        break;
    }
    ...
    try {
      switch (step) {
        case 0: { // Crear Contacto
          ...
          const payload = {
            Primer_nombre: fname,
            Segundo_nombre: document.getElementById('wz-fname2').value.trim(),
            Primer_apellido: lname,
            Segundo_apellido: document.getElementById('wz-lname2').value.trim(),
        ...
        case 1: { // Registrar Estudiante
          const payload = {
            ...
            Segundo_nombre: res.contacto.Segundo_nombre,
            Primer_apellido: res.contacto.Primer_apellido,
            Segundo_apellido: res.contacto.Segundo_apellido,
        ...
        case 2: { // Inscripción
          ...
          const payload = {
            ...
            Jornada: document.getElementById('wz-jornada').value,
  ...
  // ================================================================
  //  MODAL: GENERAR COBRO (for existing students)
  // ================================================================
  function showGenerarCobroModal(studentData) {
    ...
    overlay.innerHTML = `
      <div class="q10-modal">
        <div class="q10-modal-header">
          ...
          <button class="q10-btn-close q10-modal-close-btn" style="background:transparent;color:#6B7280">${ICONS.close}</button>
        </div>
        <div class="q10-modal-body">
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Concepto *</label>
            <input class="q10-form-input" id="q10-cobro-concepto" placeholder="Ej: Mensualidad, Material">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Valor *</label>
            <input class="q10-form-input" id="q10-cobro-valor" type="number" min="0" step="0.01" placeholder="0.00">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Fecha de Vencimiento *</label>
            <input class="q10-form-input" id="q10-cobro-fecha" type="date" value="${new Date(Date.now()+30*86400000).toISOString().split('T')[0]}">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Observaciones</label>
            <textarea class="q10-form-textarea" id="q10-cobro-obs" placeholder="Notas..."></textarea>
          </div>
        </div>
    ...
    document.getElementById('q10-panel').appendChild(overlay);
    ...
    document.getElementById('q10-cobro-submit').addEventListener('click', async () => {
      ...
      btn.disabled = true;
      btn.innerHTML = `<div class="q10-spinner" style="width:14px;height:14px;border-width:2px;display:inline-block;vertical-align:middle;"></div> Generando...`;

      try {
        await sendMsg('createOrdenPago', {
          body: {
            Codigo_estudiante: studentData.Codigo,
            Concepto: concepto,
            Valor: parseFloat(valor),
            Fecha_vencimiento: fecha,
            Observaciones: document.getElementById('q10-cobro-obs').value.trim(),
          }
        });
        ...
        showToast(err.message, 'error');
        btn.disabled = false;
        btn.innerHTML = `${icon('dollar','q10-btn-icon')} Generar`;
      }
    });
  }
  ...
  function showCreateOportunidadModal(phone, contactData) {
    ...
    overlay.innerHTML = `
      <div class="q10-modal">
        ...
        <div class="q10-modal-body">
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Nombre *</label>
            <input class="q10-form-input" id="q10-op-fname" value="${contactData?.Primer_nombre||''}" placeholder="Nombre">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Apellido *</label>
            <input class="q10-form-input" id="q10-op-lname" value="${contactData?.Primer_apellido||''}" placeholder="Apellido">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Email</label>
            <input class="q10-form-input" id="q10-op-email" type="email" value="${contactData?.Email||''}" placeholder="email@ejemplo.com">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Celular</label>
            <input class="q10-form-input" id="q10-op-phone" value="${phone||''}" placeholder="+55...">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Observaciones</label>
            <textarea class="q10-form-textarea" id="q10-op-obs" placeholder="Notas sobre el lead..."></textarea>
          </div>
        </div>
        <div class="q10-modal-footer">
          <button class="q10-btn q10-btn-outline q10-modal-cancel">Cancelar</button>
    ...
    document.getElementById('q10-op-submit').addEventListener('click', async () => {
      ...
      try {
        await sendMsg('createOportunidad', {
          body: {
            Primer_nombre: fname,
            Primer_apellido: lname,
            Email: document.getElementById('q10-op-email').value.trim(),
            Celular: document.getElementById('q10-op-phone').value.trim(),
            Observaciones: document.getElementById('q10-op-obs').value.trim()
          }
        });
  ...
  // ================================================================
  //  MODAL: CREAR CONTACTO (standalone)
  // ================================================================
  function showCreateContactoModal(phone) {
    ...
    overlay.innerHTML = `
      <div class="q10-modal">
        ...
        <div class="q10-modal-body">
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Nombre *</label>
            <input class="q10-form-input" id="q10-ct-fname" placeholder="Nombre">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Primer Apellido *</label>
            <input class="q10-form-input" id="q10-ct-lname" placeholder="Apellido">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Email</label>
            <input class="q10-form-input" id="q10-ct-email" type="email" placeholder="email@ejemplo.com">
          </div>
          <div class="q10-form-group">
            <label class="q10-form-label">Celular</label>
            <input class="q10-form-input" id="q10-ct-phone" value="${phone||''}" placeholder="+55...">
          </div>
        </div>
        <div class="q10-modal-footer">
          <button class="q10-btn q10-btn-outline q10-modal-cancel">Cancelar</button>
    ...
    document.getElementById('q10-ct-submit').addEventListener('click', async () => {
      ...
      try {
        await sendMsg('createContacto', {
          body: {
            Primer_nombre: fname,
            Primer_apellido: lname,
            Email: document.getElementById('q10-ct-email').value.trim(),
            Celular: document.getElementById('q10-ct-phone').value.trim()
          }
        });
  ...
  function showCreateActividadModal(contactData) {
    ...
    overlay.innerHTML = `
      <div class="q10-modal">
        ...
        <div class="q10-modal-body">
          <div class="q10-form-group">
            <label class="q10-form-label">Tipo *</label>
          ...
          <div class="q10-form-group">
            <label class="q10-form-label">Descripción *</label>
            <textarea class="q10-form-textarea" id="q10-act-desc" placeholder="Detalle de la interacción..."></textarea>
          </div>
        </div>
        <div class="q10-modal-footer">
          <button class="q10-btn q10-btn-outline q10-modal-cancel">Cancelar</button>
    ...
    document.getElementById('q10-act-submit').addEventListener('click', async () => {
      ...
      try {
        await sendMsg('createActividad', {
          body: {
            Tipo: document.getElementById('q10-act-type').value,
            Descripcion: desc,
            Fecha: new Date().toISOString(),
            Codigo_oportunidad: contactData?.Codigo || null,
            Observaciones: `Contacto: ${fullName(contactData)} | WhatsApp`
          }
        });
  ...
  function searchPhone(phone) {
    ...
    chrome.runtime.sendMessage({ action: 'searchPhone', phone }, (resp) => {
      if (chrome.runtime.lastError) { renderError('Erro de comunicação. Recarregue a página.'); return; }
  ...
  function onConversationChange() {
    ...
    observerDebounce = setTimeout(() => {
      ...
      if (phone && phone !== lastDetectedPhone) {
        ...
        // If wizard is running, don't auto-switch
  ...
  function init() {
    ...
    chrome.runtime.sendMessage({ action: 'checkApiKey' }, (resp) => {
      if (!resp || !resp.ok || !resp.data?.configured) renderNoApiKey();
    });
    ...
    console.log('[Q10 CRM] Extension loaded successfully (v2.0 — enrollment wizard + financials)');
  }
